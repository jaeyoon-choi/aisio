...
| NVIDIA   | MLNX\_OFED                 | 24.10-3.2.5.0        | Source        | Mellanox OFED with NVMe-oF, NFS-RDMA, and GPUDirect Storage support                                                                |
...
| Kernel   | upcie-experimental         | {{ ver_upcie_experimental }} | DKMS   | 26.04 stack: out-of-tree modules released by uPCIe; `dmabuf_import` resolves the DMA addresses of device memory, `iommu_map_pa` maps it into a VFIO-controlled device's IOMMU domain |
